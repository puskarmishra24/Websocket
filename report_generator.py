...
def generate_pdf_report(combined_results):
    ...
    try:
        ...
        style_author = ParagraphStyle('MyAuthor', parent=styles['Normal'], fontSize=16, alignment=1)
        ...
        authors = Paragraph("Under Guidance Of – Sushma E (CEH)", style_author)
...
# Solution Summary
        ...
        for url_result in combined_results.get('detailed_results', []):
            ...
            elements.append(create_bar_chart(url_vuln_counts, title=f"Vulnerability Distribution for {url_result.get('url', 'N/A')}", width = 380, height = 180))
            elements.append(Spacer(1, 30))
        ...
        print(f"\n[+] Combined PDF report generated: {absolute_path}")
